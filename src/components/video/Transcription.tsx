...
export const Transcription = ({
  ...
  const renderTranscriptionContent = () => {
    if (video.transcription_status === 'completed' && video.transcription_chunks) {
      return <div className="mt-4 space-y-2">
          ...
          {isExpanded && <div className="max-h-[150px] overflow-y-auto space-y-2">
              {video.transcription_chunks.map((chunk: TranscriptionChunk, index: number) => <div key={index} className="p-2 hover:bg-accent rounded-md cursor-pointer" onClick={() => {
  ...
  return <Card className="p-4 py-1">
      {video?.transcription_status !== 'completed' && <Button onClick={onTranscribe} disabled={isTranscribing || video?.transcription_status === 'processing'} className="w-full">
